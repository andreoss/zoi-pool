  ...
  def spec = suite("observability")(
    suite("metrics port")(
      ...
      test("nothing counted carries a url or a credential") {
        ...
        assertTrue(!snap.toString.contains("jdbc:"), snap.toString.startsWith("PoolMetricsSnapshot"))
      },
    ),
    suite("zio metrics")(
      test("pool state reaches the metric registry") {
        ...
        for {
          ...
          state <- zio.metrics.Metric.gauge("zoi_pool_connections_total").tagged("pool", "observed").value
